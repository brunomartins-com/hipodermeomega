            ...
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    @foreach($users as $user)
                        <tr>
                            <td>{{ $user->id }}</td>
                            <td class="font-w600">{{ $user->babyName }}</td>
                            <td class="font-w600">{{ \Carbon\Carbon::createFromFormat('Y-m-d', $user->babyBirthdate)->format('d/m/Y') }}</td>
                            <td class="font-w600">{{ $user->name }}</td>
                            <td class="font-w600">{{ $user->email }}</td>
                            <td class="font-w600">{{ $user->city."/".$user->state }}</td>
                            <td class="font-w600">{{ $user->photos($user->id)->count() }}</td>
                            <td class="font-w600">{{ $user->videos($user->id)->count() }}</td>
                            <td class="font-w600">
                                @if($user->active == 0)
                            ...
                            <td class="text-center">
                                ...
                                {!! Form::button('<i class="fa fa-search"></i> ', ['title'=>'Visualizar', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-info',
                                    'onclick'=>'window.open(\''.route('participantsView', $user->id).'\', \'_self\')']) !!}
                                ...
                                'data-url'=>route('participantsDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$user->id]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
