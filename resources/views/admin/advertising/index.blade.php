        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('advertisingAdd').'\', \'_self\');']) !!}
                </h3>
            </div>
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="font-w600">{{ $ad->title }}</td>
                            <td><img src="{{ $ad->image }}" height="100" alt="{{ $ad->title }}" /></td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-pencil"></i>', ['title'=>'Editar', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-primary',
                                'onclick'=>'window.open(\''.route('advertisingEdit', $ad->advertisingId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$ad->advertisingId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('advertisingId', $ad->advertisingId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('advertisingDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$ad->advertisingId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                        @endforeach
